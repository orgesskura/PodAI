...
def get_processed_files(output_path):
    return {normalize_filename(file): file for file in os.listdir(output_path) if file.endswith(('.mp3', '.part'))}
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if expected_filename in processed_files:
                    print(f"Skipping already downloaded: {processed_files[expected_filename]}")
                    return False
                elif expected_part_filename in processed_files:
                    print(f"Resuming partial download: {processed_files[expected_part_filename]}")
