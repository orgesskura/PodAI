...
def load_embeddings():
    return FAISS.load_local(
        "faiss_index_multi", OpenAIEmbeddings(), allow_dangerous_deserialization=True
    )
...
def rag():
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are an assistant that provides answers to questions based on a given context. The context is about Lex Friedman podcast."
    ...
    def chat():
        ...
        while True:
            ...
            result = chain.invoke(
                {"question": query, "history": format_history(history)}
            )
